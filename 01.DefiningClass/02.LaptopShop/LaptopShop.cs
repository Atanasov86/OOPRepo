using System;
using _02.LaptopShop;

public class LaptopShop
{
    public static void Main()
    {
        Battery battery = new Battery("Li-Ion, 4-cells, 2550 mAh", 4.5);
        Laptop lap = new Laptop
            ("Lenovo Yoga 2 Pro", "Lenovo", "Intel Core i5-4210U (2-core, 1.70 - 2.70 GHz, 3MB cache)", "8 GB", "Intel HD Graphics 4400",
            "128GB SSD", "13.3\" (33.78 cm) – 3200 x 1800 (QHD+), IPS sensor display", battery, 2259.00M);
        Console.WriteLine("Sample laptop description (full): ");
        Console.WriteLine(lap.ToString());
        Laptop lap1 = new Laptop("Lenovo Yoga 2 Pro Lenovo", 2259.00M);
        Console.WriteLine("Sample laptop description (mandatory properties only)");
        Console.WriteLine(lap1.ToString());
    }
}